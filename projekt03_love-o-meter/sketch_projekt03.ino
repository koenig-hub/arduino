...
void loop() {
  ...
  if(temperatur < ausgangsTemperatur) {
    ...
    digitalWrite(3,LOW);
    digitalWrite(4,LOW);   
  // ist die gemessene Temperatur zwischen 2 und 4 Grad höher als die Ausgangstemperatur -> LED an Pin 2 einschalten
  } else if((temperatur >= ausgangsTemperatur+2) && (temperatur < ausgangsTemperatur+4)) {
    digitalWrite(2,HIGH);
    digitalWrite(3,LOW);
    digitalWrite(4,LOW);     
  // ist die gemessene Temperatur zwischen 4 und 6 Grad höher als die Ausgangstemperatur -> LEDs an Pin 2 und 3 einschalten
  } else if((temperatur >= ausgangsTemperatur+4) && (temperatur < ausgangsTemperatur+6)) {
    digitalWrite(2,HIGH);
    digitalWrite(3,HIGH);
    digitalWrite(4,LOW);     
  // ist die gemessene Temperatur zwischen 6 und 8 Grad höher als die Ausgangstemperatur -> LEDs an Pin 2, 3 und 4 einschalten
